...
class NfcManagerPlugin: FlutterPlugin, ActivityAware, HostApiPigeon, BroadcastReceiver() {
  ...
  override fun onReceive(context: Context?, intent: Intent?) {
    ...
    val adapterState = toAdapterStatePigeonSafe(state)
    flutterApi.onAdapterStateChanged(adapterState) { /* noop */ }
  }
...
private fun toAdapterStatePigeonSafe(value: Int): AdapterStatePigeon {
  return when (value) {
    ...
    // Samsung broadcasts vendor state 5 when switching to card-only mode.
    // Reader mode is unavailable there, so expose it as OFF instead of
    // crashing or leaving the Flutter scan waiting forever.
