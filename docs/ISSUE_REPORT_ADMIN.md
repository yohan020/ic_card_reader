...
**역명 한글 표기 추가 요청**은 선택된 승차역·하차역 범위의 snapshot과 사용자가 입력한 **제안 승차역 한글 표기**·**제안 하차역 한글 표기**를 함께 보고 검토한다. 앱 데이터에 반영할 때는 자동 생성 파일이 아닌 `assets/data/stations/manual_station_names_ko.csv`에 원본 일본어 역명, 확정 한글 표기, 확인 근거를 추가한다.
